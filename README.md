...
| --- | --- | --- | --- | --- | --- |
| lightshow_example_1_elevator_music | 1 | yes | yes | 0:45 | Model X (2021+) (96%) |
| lightshow_example_2_Max_Carlisle_Auld_Lang_Syne_In_the_City | 1 | yes | yes | 1:55 | Model X (2021+) (91%) |
| lightshow_example_3_The_Arrival_5_Car | 5 | yes | yes | 1:50 | Cybertruck (100%) |
| lightshow_example_4_Ready_for_Assault_8_Car | 8 | yes | no | 1:32 | Cybertruck (85%) |
| lightshow_example_5_Cyber_Symphony_4_Car | 4 | yes | no | 1:29 | Model S (2021+) (100%) |
| lightshow_example_6_Ready_for_Assault_1_Car | 1 | no | yes | - | - |
| lightshow_example_7_Cyber_Symphony_1_Car | 1 | no | yes | - | - |

Two of the multi-car sets ship without a source because the same show is published separately as a single-car sequence: example 4 is the eight-car version of example 6, and example 5 is the four-car version of example 7. If you want to edit either, take the .xsq from the single-car archive.

"Plays fullest on" is measured, not a statement of intent: it is the vehicle that has the most of the channels the show actually drives, and it is why a show written around the [Cybertruck light bars](#cybertruck-light-bar) looks sparse elsewhere. See [when the show seems not to start](#show_looks_dead).

### <a name="arrangements"></a>A familiar name here is not the show from the car
The shows built into the car - the list in Toybox - are part of the vehicle software rather than files in this repository. Where Tesla has published a sequence behind one, it is in the table above; [asking for another](https://github.com/teslamotors/light-show/issues) is a request for the maintainers. If all you have is a .fseq, [reading an existing .fseq](#reading_an_fseq) covers importing it back into xLights as a data layer.

**An example here that shares a name with a built-in show is a different arrangement of the same track, not a copy of it.** `lightshow_example_3_The_Arrival_5_Car` is the clearest case: it is the same 1:50 track spread across five cars and written around the [Cybertruck light bars](#cybertruck_mapping), so on a Model S, Model X, Model 3 or Model Y only 17% of its lit time lands on lights the car has, against 100% on a Cybertruck. The version that plays in every vehicle cannot be that show, because it plays on every vehicle.

Where a single-car publication of a multi-car show exists it is listed separately - example 6 is the one-car *Ready for Assault* and example 7 the one-car *Cyber Symphony*. There is no equivalent single-car publication of *The Arrival* here ([#102](https://github.com/teslamotors/light-show/issues/102)).

This table is generated by `tools/examples_index.py`, and CI checks it still matches the files.
